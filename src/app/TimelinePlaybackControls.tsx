...
export function TimelinePlaybackControls({ incident }: { incident: Incident }) {
  ...
  const activeEvent =
    currentStep != null ? timeline[Math.min(currentStep, timeline.length - 1)] : null;
  ...
  const play = () => {
    ...
    if (!matchesIncident || currentStep == null || currentStep >= timeline.length - 1) {
      start(incident.id, 0);
      return;
    }
  ...
  return (
    <div className="rounded-lg border border-rose-500/35 bg-rose-950/15 px-3 py-2.5 space-y-2.5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[10px] leading-relaxed text-war-muted/90">
            Replays the curated disclosure trail in teaching order. The globe and this rail share the same milestone.
          </p>
        </div>
        <span className="shrink-0 font-mono text-[9px] uppercase tracking-[0.12em] text-war-muted">
          {currentStep == null ? `0/${timeline.length}` : `${currentStep + 1}/${timeline.length}`}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        >
          {isPlaying ? "Pause" : currentStep == null || currentStep >= timeline.length - 1 ? "Replay case" : "Resume"}
        </button>
        <button
          type="button"
          onClick={() => move(-1)}
      ...
      <div className="flex items-center gap-1.5" aria-label="Timeline milestones">
      ...
      {activeEvent && (
        <div className="border-l-2 border-rose-400 bg-black/45 px-3 py-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[10px] text-war-muted">{activeEvent.dateLabel}</span>
          </div>
          <p className="mt-1 text-[11px] font-semibold text-war-white">{activeEvent.label}</p>
          {activeEvent.detail && (
            <p className="mt-1 text-[10px] leading-relaxed text-war-muted/90">{activeEvent.detail}</p>
          )}
        </div>
      )}
    </div>
